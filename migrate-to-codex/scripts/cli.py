...
from utils.util import normalize_source_scope_root, render_toml_document, resolve_source_root
...
GENERATED_MARKER_NAME = ".migrate-to-codex-generated"
...
class ScopeDeployment:
    ...
    def has_generated_marker(
        self,
        target_path: Path,
        artifact_kind: ArtifactKind,
    ) -> bool:
        return generated_marker_path(target_path, artifact_kind).exists()
    ...
    def orphaned_codex_paths(
        ...
        for target_path in self.existing_paths(target_root, pattern):
            ...
            if relative_path not in planned_paths and self.has_generated_marker(
                target_path,
                artifact_kind,
            ):
...
def convert_scope(
    ...
    if "mcp" in components:
        result.add(
            convert_settings(
                scope,
                codex_hooks_enabled=components == DEFAULT_COMPONENTS,
            )
        )
...
def generated_marker_path(target_path: Path, artifact_kind: ArtifactKind) -> Path:
    if artifact_kind == ArtifactKind.SKILL:
        return (
            target_path / GENERATED_MARKER_NAME
            if target_path.is_dir()
            else target_path.parent / GENERATED_MARKER_NAME
        )
    if artifact_kind == ArtifactKind.AGENT:
        return target_path.with_name(f"{target_path.name}{GENERATED_MARKER_NAME}")
    return target_path.with_name(f"{target_path.name}{GENERATED_MARKER_NAME}")


def write_generated_marker(target_path: Path, artifact_kind: ArtifactKind) -> None:
    if artifact_kind not in {ArtifactKind.SKILL, ArtifactKind.AGENT}:
        return
    marker_path = generated_marker_path(target_path, artifact_kind)
    marker_path.parent.mkdir(parents=True, exist_ok=True)
    marker_path.write_text("Generated by migrate-to-codex; safe for replace-mode orphan cleanup.\n")
...
def merge_toml_documents(
    ...
    for key, generated_value in generated.items():
        ...
        if isinstance(existing_value, dict) and isinstance(generated_value, dict):
            merged[key] = merge_toml_documents(existing_value, generated_value)
        else:
            merged[key] = generated_value
    return merged


def merged_config_text(target_path: Path, generated_text: str) -> str:
    if not target_path.exists() or target_path.is_symlink():
        return generated_text

    existing_config = tomllib.loads(target_path.read_text())
    generated_config = tomllib.loads(generated_text)
    return render_toml_document(merge_toml_documents(existing_config, generated_config))
...
def write_artifact(artifact: PlannedArtifact, target_root: Path) -> None:
    ...
    if isinstance(artifact.payload, GeneratedText):
        ...
        if artifact.relative_path == CODEX_CONFIG_PATH:
            content = merged_config_text(target_path, content)
        if target_path.is_symlink():
            target_path.unlink()
        target_path.write_text(content)
        write_generated_marker(target_path, artifact.kind)
        return
...
def migration_skill_artifacts(source_root: Path) -> list[PlannedArtifact]:
    ...
    for scope_name in SCOPE_NAMES:
        ...
        for support_file in sorted(path for path in scripts_root.rglob("*") if path.is_file()):
            artifacts.append(
                PlannedArtifact.from_source_file(
                    support_file,
                    Path(scope_name)
                    ...
                    / "migrate-to-codex"
                    / "scripts"
                    / support_file.relative_to(scripts_root),
                )
            )
    return artifacts
...
def main() -> None:
    ...
    if not args.dry_run:
        ...
        if deploy_mode == DeployMode.REPLACE:
            ...
            for orphan in deployment_plan.orphaned_agent_files:
                orphan.unlink()
                marker_path = generated_marker_path(orphan, ArtifactKind.AGENT)
